...
export function SidebarSort({ fullWidth = false, className }: SidebarSortProps) {
  ...
  return (
    ...
    >
      <SelectTrigger
        chevronClassName="text-zinc-500 dark:text-zinc-400 h-5 w-5 md:h-4 md:w-4" 
        className={cn(
          "flex h-9 md:h-9 shrink-0 items-center justify-between gap-1.5 md:gap-2 rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white px-2 md:px-2.5 text-base md:text-[13px] font-medium text-zinc-700 whitespace-nowrap hover:bg-zinc-50 focus:ring-1 focus:ring-zinc-400 cursor-pointer transition-colors leading-none",
        ...
        <SelectValue placeholder="Sort order">
          {sortOrder === "new" && (
            <span className="flex items-center gap-1.5 md:gap-2 text-zinc-500">
              <CalendarArrowUp className="size-5 md:size-4 shrink-0" />
              <span>New</span>
            </span>
          )}
          {sortOrder === "hot" && (
            <span className="flex items-center gap-1.5 md:gap-2 text-zinc-500">
              <Flame className="size-5 md:size-4 shrink-0" />
              <span>Hot</span>
            </span>
          )}
          {sortOrder === "top" && (
            <span className="flex items-center gap-1.5 md:gap-2 text-zinc-500">
              <ArrowUp className="size-5 md:size-4 shrink-0" />
              <span>Top</span>
            </span>
          )}
          {sortOrder === "controversial" && (
            <span className="flex items-center gap-1.5 md:gap-2 text-zinc-500">
              <Swords className="size-5 md:size-4 shrink-0" />
